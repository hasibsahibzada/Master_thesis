...
\emph{Implicit interaction phase} is the phase where the system detects the person’s position and projects notification when user passes. In this phase it could also presents a kind of representation of the person so that the passersby can see the reaction to be convinced to enter the subtle interaction phase. The \emph{Subtle mode} activates when user give implicit hints like stopping by screen. In this mode a detailed notification or state is shown in which user can start interacting with the content and multiple users can interact too. But when exploring more personal content then the users moves closer to the screen to enable \emph{Personal interaction phase}, in this phase the user is very close to the screen and the interaction could be done by touching the screen and exploring more personal contents. 
